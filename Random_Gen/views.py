        ...
        if request.method == "POST":
            ...
            if n > 0 and m > 0:
                ...
                while len(seq) < n - 1:
                    r = random.randint(
                        -m, m - 1
                    )  # Generate a random number between -m and m-1 so that the number is not in the sequence
                    if r not in seq:
                        seq.append(r)
                seq.insert(
                    random.randint(0, n - 1), m
                )  # Appending the maximum value to the sequence at a index randomly chosen
